...
public class GridMaker : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            // SceneManager.LoadScene(SceneManager.GetSceneByName("Pause"));
        }
    }
    ...
    public void CreateGrid()
    {
        ...
        {
            if (levelHolder[currentLevel].level[i] != ElementType.Empty)
            {
                GameObject g = Instantiate(cellHolder, new Vector3(counter % cols, counter / rows, 0), Quaternion.identity);
                cells.Add(g);
                ElementType currentElement = levelHolder[currentLevel].level[i];
    ...
    public Sprite ReturnSpriteOf(ElementType e)
    {
        ...
        for (int i = 0; i < cells.Count; i++)
        {
            if (cells[i] != null)
            {
                ...
                if (IsElementStartingWord(currentcell.Element))
                {
                    ...
                    /*if (DoesListContainElement(FindObjectsAt(currentcell.CurrentRow + 1, currentcell.CurrentCol), ElementType.IsWord))
                    {
                    ...
                    if (DoesListContainElement(FindObjectsAt(currentcell.CurrentRow, currentcell.CurrentCol + 1), ElementType.IsWord))
                    {
    ...
    public ElementType GetActualObjectFromWord(ElementType e)
    {
        if (e == ElementType.PoopWord)
        {
            return ElementType.Poop;
        }
        else if (e == ElementType.PQWord)
        {
            return ElementType.PQ;
        }
        else if (e == ElementType.ToiletWord)
        {
            return ElementType.Toilet;
        }
        else if (e == ElementType.WallWord)
        {
            return ElementType.Wall;
        }
        else if (e == ElementType.WaterWord)
        {
    ...
    public void Rule(ElementType a, ElementType b)
    {
        ...
        else if ((int)b >= 150)
        {
            ...
            else if (b == ElementType.PushWord)
            {
            ...
            else if (b == ElementType.StopWord)
            {
                //stop property true
                foreach (CellProperty p in GetAllCellsOf(GetActualObjectFromWord(a)))
                {
            ...
            else if (b == ElementType.SinkWord)
            {
                foreach (CellProperty p in GetAllCellsOf(GetActualObjectFromWord(a)))
                {
                    p.IsItDestroy(true);
    ...
    public CellProperty GetCellOf(ElementType e)
    {
        foreach (GameObject g in cells)
        {
    ...
    public List<CellProperty> GetAllCellsOf(ElementType e)
    {
    ...
    public GameObject GetFirstPushableObjectAt(int r, int c)
    {
        List<GameObject> objectsAtRC = FindObjectsAt(r, c);
    ...
    public bool IsElementStartingWord(ElementType e)
    {
    ...
    public ElementType ReturnWordAt(int r, int c)
    {
        List<GameObject> l = FindObjectsAt(r, c);

        foreach (GameObject g in l)
        {
            ElementType e = g.GetComponent<CellProperty>().Element;
            if ((int)e >= 100)
            {
                return e;
            }
    ...
    public bool DoesListContainElement(List<GameObject> l, ElementType e)
    {
        foreach (GameObject g in l)
        {
    ...
    public bool IsElementIsWord(ElementType e)
    {
        if ((int)e == 99)
        {
            return true;
